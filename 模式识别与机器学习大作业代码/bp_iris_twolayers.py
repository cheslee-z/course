...
def initialize_network(n_inputs, n_hidden, n_hidden_s,n_outputs):
    ...
    # 输出层
    output_layer = [{'weights': [random.random() for i in range(n_hidden_s + 1)]} for i in range(n_outputs)]
    network.append(output_layer)
    return network


def train(train_data, l_rate, epochs, n_hidden, n_hidden_s,val_data):
    """训练神经网络（迭代n_epoch个回合）
    ...
    # 初始化网络
    network = initialize_network(n_inputs, n_hidden, n_hidden_s, n_outputs)
...
if __name__ == "__main__":
    ...
    # 训练模型
    network = train(train_data, l_rate, epochs, n_hidden, n_hidden_s, val_data)
