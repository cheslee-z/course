...
def initialize_network(n_inputs, n_hidden, n_hidden_s,n_hidden_ss,n_outputs):
    """初始化BP网络（初始化隐藏层和输出层的参数：权重矩阵和偏置项）
    :param n_inputs: 特征列数
    :param n_hidden: 第一个隐藏层神经元个数
    :param n_hidden_s: 第二个隐藏层神经元个数
    :param n_hidden_ss: 第三个隐藏层神经元个数
    ...
    # 第二个隐藏层
    hidden_layer_s = [{'weights': [random.random() for i in range(n_hidden + 1)]} for i in range(n_hidden_s)]
    network.append(hidden_layer_s)
    # 第三个隐藏层
    hidden_layer_ss = [{'weights': [random.random() for i in range(n_hidden_s + 1)]} for i in range(n_hidden_ss)]
    network.append(hidden_layer_ss)
    # 输出层
    output_layer = [{'weights': [random.random() for i in range(n_hidden_ss + 1)]} for i in range(n_outputs)]
    network.append(output_layer)
    return network


def train(train_data, l_rate, epochs, n_hidden, n_hidden_s,n_hidden_ss,val_data):
    """训练神经网络（迭代n_epoch个回合）
    ...
    # 初始化网络
    network = initialize_network(n_inputs, n_hidden, n_hidden_s,n_hidden_ss, n_outputs)
...
if __name__ == "__main__":
    ...
    l_rate = 0.1  # 学习率
    epochs = 300  # 迭代训练的次数
    n_hidden = 4  # 第一个隐藏层神经元个数
    n_hidden_s = 4  # 第二个隐藏层神经元个数
    n_hidden_ss = 4  # 第三个隐藏层神经元个数
    n_train_data = 130  # 训练集的大小（总共150条数据，训练集130条，验证集20条）

    # 加载数据并划分训练集和验证集
    train_data, val_data = load_dataset(file_path, n_train_data)
    # 训练模型
    network = train(train_data, l_rate, epochs, n_hidden, n_hidden_s, n_hidden_ss,val_data)
