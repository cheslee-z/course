...
for j = 1:2:imagesNo
   train_labels{(j+1)/2} = y1(j);
end

%% 读取测试数据
imagesNo = 18;
for i = 1: 2:imagesNo/2
    eeg = eval(strcat('edf2',num2str(i)));  
    eeg = eeg / max(eeg(:));
    test_data{(i+1)/2} = eeg;
end
for i = imagesNo/2+1:2:imagesNo
    eeg = eval(strcat('edf2',num2str(i)));  
    eeg = eeg / max(eeg(:));
    test_data{i/2+1} = eeg;
end
...
for j = 1:2:imagesNo/2
   test_labels{(j+1)/2} = y2(j);
end
for j = imagesNo/2+1:2:imagesNo
    test_labels{j/2+1} = y2(j);
end
disp('EDF数据成功读取');
...
train_y = zeros(2,sizeTrain);
test_y = zeros(2,sizeTest);
